...
variable "min_instances" {
  type        = number
  default     = 0
  description = "Minimum number of container instances allowed to start."
}
...
variable "startup_probe_http" {
  description = "HTTP GET probes for startup"
  type = list(object({
    port = number
    path = string
  }))
...
variable "startup_probe_grpc" {
  description = "gRPC probes for startup"
  type = list(object({
    service = string
  }))
  default = []
...
variable "liveness_probe_http" {
  description = "HTTP GET probes for liveness"
  type = list(object({
    port = number
    path = string
  }))
...
variable "liveness_probe_grpc" {
  description = "gRPC probes for liveness"
  type = list(object({
    service = string
  }))
  default = []
